...
class AnomalyDetector {
  static List<SpendingInsight> detect(List<Expense> expenses) {
    ...
    final currency = expenses.firstOrNull?.currency ?? 'INR';
    final now = DateTime.now();
    final thisWeekStart = now.subtract(Duration(days: now.weekday - 1));
    final thisWeekStartDay = DateTime(
      thisWeekStart.year,
      thisWeekStart.month,
      thisWeekStart.day,
    );
      ...
      if (thisWeekAmt > weeklyAvg * 1.5) {
        ...
        insights.add(
          SpendingInsight(
            ...
            body:
                '${formatAmount(thisWeekAmt, currency)} this week — $multiplier× your usual ${formatAmount(weeklyAvg, currency)}',
