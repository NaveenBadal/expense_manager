...
class InsightsService {
  static List<SpendingInsight> compute(List<Expense> expenses) {
    ...
    final currency = expensesOnly.first.currency;

    // Weekend vs weekday ratio
    final weekend = expensesOnly
        .where((e) => e.date.weekday >= 6)
        .fold(0.0, (s, e) => s + e.amount);
    final weekday = expensesOnly
        .where((e) => e.date.weekday < 6)
        .fold(0.0, (s, e) => s + e.amount);
    if (weekday > 0) {
      ...
      if (ratio > 0.5) {
        insights.add(
          SpendingInsight(
            ...
            body:
                'You spend ${(ratio * 100).toStringAsFixed(0)}% as much on weekends vs weekdays.',
    ...
    if (dowTotals.isNotEmpty) {
      final maxEntry = dowTotals.entries.reduce(
      ...
      insights.add(
        SpendingInsight(
          ...
          body:
              '${formatAmount(maxEntry.value, currency)} total on ${dayName}s.',
    ...
    final older = expensesOnly.where(
      (e) => e.date.isAfter(cutoff60) && !e.date.isAfter(cutoff30),
    );

    final recentByCategory = <String, double>{};
    for (final e in recent) {
      recentByCategory.update(
        e.category,
        (v) => v + e.amount,
        ifAbsent: () => e.amount,
      );
    ...
    if (fastestCat != null) {
      insights.add(
        SpendingInsight(
          ...
          title:
              '$fastestCat spending up ${(maxGrowth * 100).toStringAsFixed(0)}%',
