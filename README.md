# Fund Flow

Fund Flow is an AI-first personal financial agent built with Flutter. It turns
supported transaction SMS into a local, queryable financial record, then uses
verified tools to answer questions and complete user-approved actions.

## Product and design source of truth

Read [`docs/greenfield_rebuild.md`](docs/greenfield_rebuild.md) before changing
any user-facing experience. It is the sole product, architecture, privacy, and
Current design-language contract. The previous implementation is not a design
or code reference.

## Development

Useful Flutter references:

- [Learn Flutter](https://docs.flutter.dev/get-started/learn-flutter)
- [Write your first Flutter app](https://docs.flutter.dev/get-started/codelab)
- [Flutter learning resources](https://docs.flutter.dev/reference/learning-resources)

For help getting started with Flutter development, view the
[online documentation](https://docs.flutter.dev/), which offers tutorials,
samples, guidance on mobile development, and a full API reference.
